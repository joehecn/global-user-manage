...
import getEnforcer from './enforcer/index.js'
import MyAuthorizer from './classes/MyAuthorizer.js'

let _res = null

export const init = (rbacPgPool, modelPath) => {
  if (_res) return _res
  ...
  const addGroupingPolicy = async g => {
    const e = await enforcer()
  ...
  // [sub_user_id, sub_role_id, dom_id]
  // [ObjectId, ObjectId, ObjectId]
  const removeGroupingPolicy = async g => {
    const e = await enforcer()
  ...
  // 设置角色所有权限
  const setRolePermissions = async (sub, dom, policys) => {
    const e = await enforcer()
    ...
    // 再添加权限
    // policys: [[obj, act]]
    const rules = policys.map(p => [sub, dom, ...p])
  ...
  // 获取角色的权限列表
  const getPermissionsForRole = async (sub, dom) => {
    const e = await enforcer()
